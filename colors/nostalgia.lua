-- nostalgia: Jon Blow's Emacs "nostalgia" palette, ported from
-- focus/config/themes/jblow-nostalgia.focus-theme.
...
vim.g.colors_name = "nostalgia"

local hl = vim.api.nvim_set_hl

local c = {
  back = "#292929",       -- background0
  panel = "#1a1a1a",      -- background1..4, bars, floats
  panel_deep = "#131313", -- build_panel_background
  ruler = "#161616",      -- ruler / cursor_inactive / active_pane_border

  -- selection_active is literally 0000FF upstream; it's the signature look.
  selection = "#0000ff",
  -- Alpha colors from the focus theme, pre-blended over their backdrop.
  soft_highlight = "#484648", -- FCEDFC26 over back: brackets, indent guides, inactive search
  list_cursor = "#51483c",    -- D3B58D4C over panel
  list_hover = "#2c2925",     -- D3B58D19 over panel
  search_active = "#cd6889",

  cursor = "#89e2a1",
  at_cursor = "#1a1a1a",

  text = "#d3b58d",      -- code_default (tan)
  identifier = "#bfc9db", -- code_identifier / ui_default
  comment = "#ffff00",    -- code_comment (yellow — the hallmark)
  comment_multi = "#87919d",
  keyword = "#ffffff",
  type = "#98fb98",
  string = "#bebebe",
  value = "#7fffd4",     -- aquamarine: bools, builtin constants
  number = "#d699b5",
  ...
  diff_add = "#273927",    -- region_addition over back
  diff_delete = "#402727", -- region_deletion over back
  diff_change = "#1a5152", -- region_header
  code_add = "#33b333",
  code_del = "#e64d4d",
}
...
hl(0, "CursorCommand", { fg = c.at_cursor, bg = c.value })
vim.opt.guicursor = {
...
hl(0, "FloatBorder", { fg = c.ui_neutral, bg = c.panel })
hl(0, "FloatTitle", { fg = c.identifier, bg = c.panel, bold = true })

hl(0, "CursorLine", { bg = c.panel })
hl(0, "CursorColumn", { bg = c.panel })
...
hl(0, "StatusLineCommand", { fg = c.at_cursor, bg = c.value, bold = true })
...
hl(0, "ModeMsg", { fg = c.identifier })
hl(0, "MoreMsg", { fg = c.identifier })
hl(0, "Question", { fg = c.value })
hl(0, "ErrorMsg", { fg = c.error })
hl(0, "WarningMsg", { fg = c.ui_warning })

hl(0, "NonText", { fg = c.ui_neutral })
hl(0, "SpecialKey", { fg = c.ui_neutral })
...
-- jblow's comments are yellow and upright, not italic.
...
hl(0, "Todo", { fg = c.at_cursor, bg = c.comment, bold = true })
...
hl(0, "DiagnosticVirtualTextWarn", { fg = c.warning, bg = "#3a3323" })
...
hl(0, "DiagnosticSignInfo", { fg = c.identifier, bg = c.panel })
hl(0, "DiagnosticSignHint", { fg = c.ui_dim, bg = c.panel })

hl(0, "GitSignsAdd", { fg = c.code_add, bg = c.panel })
hl(0, "GitSignsChange", { fg = c.warning, bg = c.panel })
...
hl(0, "DiffText", { bg = c.diff_change, bold = true })

hl(0, "TelescopeNormal", { fg = c.text, bg = c.back })
hl(0, "TelescopeBorder", { fg = c.ui_neutral, bg = c.back })
hl(0, "TelescopePromptNormal", { fg = c.text, bg = c.back })
hl(0, "TelescopePromptBorder", { fg = c.ui_neutral, bg = c.back })
...
hl(0, "TelescopeResultsBorder", { fg = c.ui_neutral, bg = c.back })
hl(0, "TelescopeResultsTitle", { fg = c.ui_dim, bg = c.back })
hl(0, "TelescopePreviewNormal", { fg = c.text, bg = c.back })
hl(0, "TelescopePreviewBorder", { fg = c.ui_neutral, bg = c.back })
...
hl(0, "TelescopeMultiSelection", { fg = c.value, bg = c.list_hover })
hl(0, "TelescopeMatching", { fg = c.highlight, bold = true })

hl(0, "HarpoonBorder", { fg = c.ui_neutral })
...
-- entries is the full cycle. Upstream leaves region_scope_* flat at #292929; this
-- lifts each nesting level slightly warmer off that base.
local scope_bgs = {
  "#292929",
  "#2e2d2b",
  "#33312d",
