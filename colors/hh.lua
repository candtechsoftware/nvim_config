...
-- (statusline reads M.colors / M.get_cursor_color).
vim.g.hh_theme = M

M.setup()

-- The nested-scope engine and the project #define indexer used to live in this
-- file. They are modules now — see lua/hh/scope.lua and lua/hh/macros.lua.
-- A colors/*.lua chunk is re-sourced on every :colorscheme, which reset their
-- caches, timers and match-id tables while the autocmds and window matches they
-- had already created stayed live. As require'd modules that state is cached
-- once and survives a colorscheme reload.
require("hh.scope").setup({ cycle_len = #colors.back_cycle })
